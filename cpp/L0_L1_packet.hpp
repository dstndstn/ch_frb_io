...
struct L0_L1_header {
    ...
    // If positive, must equal (nbeam * nfreq_coarse * nupfreq * ntsamp)
    ...
    uint16_t    nfreq_coarse;   // number of "FPGA" frequency channels between 0 and 1024, byte offset 18
    ...
    uint16_t    coarse_freq_ids[nfreq_coarse];  // coarse_freq_ids are between 0 and 1024 for full chime, byte offset of array is (24+2*nbeam)
    ...
    // coarse frequency.  This increases the header size so that the header is ~4% of the total
    ...
    // 8-bit encoded data, as a 4D array with shape (nbeam, nfreq_coarse, nupfreq, ntsamp).
    ...
    uint8_t     data[ abs(data_nbytes) ];   // byte offset (24 + 2*nbeam + 2*nfreq_coarse + 4*nbeam*nfreq_coarse)
};
